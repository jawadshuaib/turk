...
Turk is an AI-powered autonomous QA testing platform. Each "turk" is an isolated Docker container running an [OpenClaw](https://openclaw.ai/) agent with a headless browser, an LLM brain (via local Ollama or Ollama Cloud), and a specific testing role. A Next.js web dashboard manages turks, credentials, projects, and streams live results via WebSocket.
...
| `OLLAMA_API_KEY` | Ollama Cloud API key (optional — can also be set from the UI) | *(empty)* |
...
│       │   ├── api/turks/[id]/     # start, stop, pause, resume, findings
│       │   ├── api/projects/       # Project CRUD
│       │   ├── api/ollama/         # models, pull, cloud-models
│       │   └── api/settings/       # App settings (API keys, etc.)
│       ├── components/
...
4. Enter the target URL to test
5. Write testing instructions — be specific:
   ```
